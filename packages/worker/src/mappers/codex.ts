...
/**
 * Codex stores sessions as JSONL. The stable records for display are
 * response_item entries: user/assistant messages, function calls, and function
 * call outputs. Encrypted reasoning entries are intentionally skipped.
 */
	...
	for (const entry of parseLines(raw)) {
		...
		if (payload.type === "message") {
			...
			if (payload.role === "user") {
				flushAssistant();
				messages.push({ role: "human", content, timestamp: entry.timestamp });
				continue;
			}
		...
		if (payload.type === "function_call") {
			...
			if (payload.call_id) pendingToolCalls.set(payload.call_id, toolCall);
			continue;
		}
